...
with elements_sdk.ApiClient(configuration) as api_client:
    ...
    tape = 1 # int | Filter the returned list by `tape`. (optional)
    ...
    # and optional values
    try:
        api_response = api_instance.get_all_archived_file_entries(id=id, is_dir=is_dir, name=name, fullpath=fullpath, parent=parent, tape=tape, ordering=ordering, limit=limit, offset=offset)
        pprint(api_response)
    except elements_sdk.ApiException as e:
