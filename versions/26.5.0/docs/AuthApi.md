...
[**get_access_token_media_library_permissions**](AuthApi.md#get_access_token_media_library_permissions) | **GET** `/api/2/auth/access-tokens/{id}/permissions` | 
...
[**EvaluatePasswordEndpointResponse**](EvaluatePasswordEndpointResponse.md)
...
# **get_access_token_media_library_permissions**
    def [MediaRootPermission] get_access_token_media_library_permissions(id)
...
with elements_sdk.ApiClient(configuration) as api_client:
    ...
    # example passing only required values which don't have defaults set
    try:
        api_response = api_instance.get_access_token_media_library_permissions(id)
        pprint(api_response)
    except elements_sdk.ApiException as e:
        print("Exception when calling AuthApi->get_access_token_media_library_permissions: %s\n" % e)
```
